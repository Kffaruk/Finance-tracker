...
export default function Register() {
  ...
  const onSubmit = async (data) => {
    setLoading(true)
    try { await authRegister({ name: data.name, email: data.email, password: data.password }); navigate('/') }
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-50 via-white to-blue-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">বিনামূল্যে অ্যাকাউন্ট তৈরি করুন</p>
        </div>
        ...
        <div className="card p-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <input className="input" placeholder="আপনার নাম"
                {...register('name', { required: 'নাম দিন', minLength: { value: 2, message: 'নাম কমপক্ষে ২ অক্ষরের হতে হবে' } })} />
            ...
            <div>
              ...
              <div className="relative">
                <input className="input pr-10" type={showPw ? 'text' : 'password'} placeholder="কমপক্ষে ৬ অক্ষর"
            ...
            <div>
              ...
              <input className="input" type="password" placeholder="পাসওয়ার্ড আবার লিখুন"
                {...register('confirm', { required: 'নিশ্চিত করুন', validate: v => v === pw || 'পাসওয়ার্ড মিলছে না' })} />
              {errors.confirm && <p className="form-error">{errors.confirm.message}</p>}
            </div>

            <button type="submit" disabled={loading} className="btn-primary w-full py-3">
              {loading ? <Spinner size="sm" /> : 'অ্যাকাউন্ট তৈরি করুন'}
            </button>
          </form>

          <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-4">
            অ্যাকাউন্ট আছে?{' '}
            <Link to="/login" className="text-primary-600 dark:text-primary-400 font-medium hover:underline">লগইন করুন</Link>
          </p>
        </div>
      </div>
    </div>
